...
export class PlaylistEngine {
  ...
  /**
   * Generate mock recommendation for frontend
   * In production, would call Spotify Web API
   */
  static generateMockRecommendation(mood: MoodProfile): PlaylistRecommendation {
    const moodCategory = this.analyzeMoodProfile(mood);
    const playlistName = this.generatePlaylistName(mood);
    const description = this.generateDescription(mood);

    // Generate mock tracks based on mood
    const mockTracks = this.generateMockTracks(moodCategory, 10);
  ...
  /**
   * Generate mock tracks for each mood category
   */
  private static generateMockTracks(category: string, count: number) {
    const trackPool: Record<string, Array<{ name: string; artist: string; album: string }>> = {
      [this.MOOD_CATEGORIES.ENERGETIC]: [
        { name: 'High Energy Track 1', artist: 'Artist A', album: 'Energy Mix' },
        { name: 'High Energy Track 2', artist: 'Artist B', album: 'Power Up' },
        { name: 'High Energy Track 3', artist: 'Artist C', album: 'Adrenaline' }
      ],
      [this.MOOD_CATEGORIES.CALM]: [
        { name: 'Calm Ambient 1', artist: 'Ambient Artist', album: 'Peaceful Sounds' },
        { name: 'Calm Ambient 2', artist: 'Chill Maestro', album: 'Relaxation' },
        { name: 'Calm Ambient 3', artist: 'Zen Master', album: 'Mindfulness' }
      ],
      [this.MOOD_CATEGORIES.HAPPY]: [
        { name: 'Happy Pop 1', artist: 'Pop Star', album: 'Feel Good' },
        { name: 'Happy Pop 2', artist: 'Joy', album: 'Positive Vibes' },
        { name: 'Happy Pop 3', artist: 'Smile', album: 'Sunshine' }
      ]
    };

    const default_tracks = [
      { name: 'Track 1', artist: 'HarmonyArtist', album: 'Harmonic Waves' },
      { name: 'Track 2', artist: 'MoodMaster', album: 'Emotional Journey' },
      { name: 'Track 3', artist: 'SonicHealer', album: 'Sound Therapy' }
    ];

    const tracks = trackPool[category] || default_tracks;
    ...
    return Array(count).fill(null).map((_, i) => {
      ...
      return {
        ...
        artist: track.artist,
        album: track.album,
        uri: `spotify:track:${Date.now()}_${i}`
      };
    });
  }
